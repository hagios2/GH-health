...
//        $this->middleware('auth:admin');
    ...
    public function getStats(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $victims_report = $this->fetchVictimStats($start_date, $end_date, $request)->groupBy('Year(created_at)');

        $product_report = $this->fetchDistrictProductStats($start_date, $end_date, $request)->groupBy('Year(created_at)');

        $reported_cases = $this->fetchReportedCases($start_date, $end_date, $request)->groupBy('Year(created_at)');
